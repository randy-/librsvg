...
rsvg_propagate_error (GError ** err,
                      ...
                      gint code)
{
        /* Keep the first error; never overwrite. */
...
static void
svg_context_free (SvgContext *context)
{
        if (context == NULL)
                return;
        if (context->handle)
...
gdk_pixbuf__svg_image_stop_load (gpointer data, GError **error)
{
        ...
        pixbuf = rsvg_handle_get_pixbuf_and_error (context->handle, error);
        if (pixbuf != NULL) {
