...
_rsvg_node_rect_draw (RsvgNode * self, RsvgDrawingCtx * ctx, int dominate)
{
    ...
    /* Negative width/height: still take abs (legacy). Negative rx/ry
     * are invalid and ignored (SVG2 / 2.62; bug718), same as Auto. */
    ...
    {
        gboolean have_rx = rect->got_rx && rx >= 0.;
        gboolean have_ry = rect->got_ry && ry >= 0.;

        if (!have_rx && !have_ry) {
            rx = 0.;
            ry = 0.;
        } else if (have_rx && !have_ry) {
            ry = rx;
        } else if (!have_rx && have_ry) {
            rx = ry;
        }
    ...
    rsvg_state_reinherit_top (ctx, self->state, dominate);
    ctx->suppress_markers = TRUE;
    rsvg_render_path (ctx, path);
    ctx->suppress_markers = FALSE;
    rsvg_cairo_path_destroy (path);
}
