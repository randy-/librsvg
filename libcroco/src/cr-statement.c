...
/* Standalone parse-from-buf: SAC start_* stores a statement on the
 * handler. CRParser's production parsers do not call
 * unrecoverable_error (only the full stylesheet path does), so a
 * failed production leaked that statement. Invoke the handler's
 * cleanup, then drop our extra handler ref after parser_destroy. */
static void
standalone_fail_unrecoverable (CRDocHandler * a_handler)
{
        if (a_handler && a_handler->unrecoverable_error)
                a_handler->unrecoverable_error (a_handler);
}

static void
standalone_drop_parser (CRParser ** a_parser, CRDocHandler ** a_handler)
{
        if (a_parser && *a_parser) {
                cr_parser_destroy (*a_parser);
                *a_parser = NULL;
        }
        if (a_handler && *a_handler) {
                cr_doc_handler_unref (*a_handler);
                *a_handler = NULL;
        }
}
                                     ...
                                     enum CREncoding a_enc)
{
        ...
        if (status != CR_OK || result == NULL) {
                standalone_fail_unrecoverable (sac_handler);
                result = NULL;
        }

        standalone_drop_parser (&parser, &sac_handler);
        return result;
}
