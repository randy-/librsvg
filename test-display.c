...
render_to_surface (ViewerCbInfo *info)
{
...

    {
        ...
        if (info->id)
            ok = rsvg_handle_render_layer (info->handle, cr, info->id, &viewport, NULL);
        else
            ok = rsvg_handle_render_document (info->handle, cr, &viewport, NULL);
        if (!ok) {
            cairo_destroy (cr);
            cairo_surface_destroy (surface);
            return NULL;
		   ...
		   gpointer           user_data)
{
    ...
    cairo_save (cr);
    {
        ...
        rsvg_handle_render_document (info->handle, cr, &viewport, NULL);
    }
    cairo_restore (cr);
}
...
main (int argc, char **argv)
{
...

    {
        ...
        if (!rsvg_handle_get_intrinsic_size_in_pixels (info.handle, &w, &h)
            || w <= 0 || h <= 0) {
            RsvgRectangle ink = { 0, 0, 0, 0 };

            rsvg_handle_get_geometry_for_element (info.handle, NULL, &ink, NULL, NULL);
            w = ink.width;
            h = ink.height;
        }
        if (w <= 0)
        ...
        if (h <= 0)
            h = 1;
        info.dimensions.width = (int) (w + 0.5);
        info.dimensions.height = (int) (h + 0.5);
        info.dimensions.em = info.dimensions.width;
        info.dimensions.ex = info.dimensions.height;
    }
